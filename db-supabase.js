...
// ---- staff access (see staff_members in supabase/schema.sql) ----
// Both of these call security-definer SQL functions, so they work for any
// signed-in user regardless of what RLS would otherwise let them read from
// staff_members directly.
async isAuthorizedStaff() {
return unwrap(await client.rpc("is_authorized_staff"));
},
async isAdmin() {
return unwrap(await client.rpc("is_staff_admin"));
},
async listStaff() {
return unwrap(await client.from("staff_members").select("*").order("created_at", { ascending: true }));
},
async addStaff(email, role, addedBy) {
return unwrap(
await client
.from("staff_members")
.insert({ email: email.trim().toLowerCase(), role: role, added_by: addedBy || null })
.select()
);
},
async setStaffRole(email, role) {
return unwrap(await client.from("staff_members").update({ role: role }).eq("email", email).select());
},
async removeStaff(email) {
return unwrap(await client.from("staff_members").delete().eq("email", email));
},
